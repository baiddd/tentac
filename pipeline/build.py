...
no ending punctuation) capturing the single most notable AI story this week \
for a practitioner digest. No hype, no "X: Y" colon-subtitle construction. \
If nothing stands out, write a plain title saying the week was quiet.
...
def write_title(items: list[ScoredItem]) -> str:
    ...
    words), distinct from write_headline's longer sentence. Mirrors
    write_headline in every other way, including the no-hype/no-colon-
    subtitle constraint and the quiet-week fallback.
    """
    client = Anthropic()
    response = client.messages.create(
        model="claude-sonnet-5",
...
def build_issue(
    ...
    for a fully-automated path with a real ANTHROPIC_API_KEY. `title` works
    the same way: a short punchy title (a few words) distinct from the
    longer `headline` sentence, given as-is if supplied, or generated by
    `write_title` (another LLM call) if omitted. `analyzed_by`, if given, is recorded
